...
struct CudaMemoryPlan {
    ...
    std::uint64_t scratch_ids_bytes = 0;  // int[batch * k]
    std::uint64_t scratch_dist_bytes = 0; // double[batch * k]
...
inline CudaMemoryPlan make_cuda_memory_plan(std::uint64_t n, std::uint64_t k,
    ...
    if (n > static_cast<std::uint64_t>(std::numeric_limits<int>::max()) ||
        cells > static_cast<std::uint64_t>(std::numeric_limits<int>::max() - 1)) {
    ...
    if (detail::mul_overflow(batch_queries, k, &batch_k) ||
        ...
        plan.error = "per-batch scratch allocation size overflows uint64_t";
        return plan;
    }
    plan.total_bytes = 0;
